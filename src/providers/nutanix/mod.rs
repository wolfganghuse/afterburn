...
impl NutanixProvider {
    pub fn try_new() -> Result<NutanixProvider> {
        let client = retry::Client::try_new()?;
        let data: NutanixProvider = client
            .get(
                retry::Json,
    ...
    fn parse_attrs(&self) -> Vec<(String, String)> {
        let mut attrs = vec![
            ("NUTANIX_HOSTNAME".to_owned(), self.hostname.clone()),
            ("NUTANIX_REGION".to_owned(), self.region.clone()),
        ];
        ...
        if let Some(ref ifaces) = self.interfaces.private {
            for (i, a) in ifaces.iter().enumerate() {
                if let Some(ref v4) = a.ipv4 {
                    attrs.push((
                        format!("NUTANIX_IPV4_PRIVATE_{}", i),
                        format!("{}", v4.ip_address),
                    ));
                }
                if let Some(ref v6) = a.ipv6 {
                    attrs.push((
                        format!("NUTANIX_IPV6_PRIVATE_{}", i),
                        format!("{}", v6.ip_address),
                    ));
                }
            }
    ...
    fn parse_interfaces(&self, interfaces: Vec<Interface>) -> Result<Vec<network::Interface>> {
        ...
        for iface in interfaces {
            ...
            let (mut addrs, mut routes) = NutanixProvider::parse_interface(&iface)?;
...
impl MetadataProvider for NutanixProvider {
